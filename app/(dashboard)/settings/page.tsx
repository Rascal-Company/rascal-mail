...
import { organizationSchema, type OrganizationFormData } from '@/lib/utils/validators';
...
export default function SettingsPage() {
  ...
  const onSubmit = async (data: OrganizationFormData) => {
    ...
    try {
      ...
      toast({ title: 'Virhe', description: 'Tallentaminen epäonnistui', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-muted-foreground">Hallitse organisaation asetuksia</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>Käytetään oletuksena uusissa kampanjoissa</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="from_name">Lähettäjän nimi</Label>
              <Input id="from_name" {...register('from_name')} placeholder="Yritys Oy" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="from_email">Lähettäjän sähköposti</Label>
              <Input id="from_email" type="email" {...register('from_email')} placeholder="noreply@yritys.fi" />
            ...
            <div className="space-y-2">
              ...
              <Input id="reply_to_email" type="email" {...register('reply_to_email')} placeholder="info@yritys.fi" />
              {errors.reply_to_email && <p className="text-sm text-destructive">{errors.reply_to_email.message}</p>}
            </div>
          </CardContent>
        </Card>
